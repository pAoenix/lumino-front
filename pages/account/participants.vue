<template>
  <view class="participants">
    ...
    >
      <up-checkbox
        class="participants-checkbox-group"
        v-for="(item, index) in userShowList"
        shape="circle"
        :key="index"
        activeColor="#55c9c9"
        :label="item.name"
...
export default {
  ...
  onShow: function () {
    ...
    console.log(data, "-------<<<<<<<<<<");
    let newUsers = data.accountUser.some((item) => item.id == userInfo.id)
      ? data.accountUser.concat([userInfo])
      : data.accountUser;
    ...
    this.accountUserUrls = this.accountUser.map(
      (item) => newUsersSele.find((items) => items.id == item)?.icon_url || ""
    );
    this.userShowList = [];
    users.forEach((item) => {
      if (
        data.accountBookUser?.length &&
        data.accountBookUser.some((items) => item.id == items)
      ) {
  ...
  methods: {
    checkboxChange(data) {
      ...
      this.accountUserUrls = data.map(
        (item) => newUsers.find((items) => items.id == item)?.icon_url || ""
      );
    },
    allCheckedChange(data) {
      if (data) {
        ...
        this.accountUserUrls = this.userShowList.map(
          (item) =>
            newUsers.find((items) => items.id == item.id)?.icon_url || ""
        );
        console.log(this.accountUserUrls);
      } else {
        this.accountUser = [];
