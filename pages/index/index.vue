<template>
			...
			<view v-for="(account, ind) in accountList" :key="ind">
				...
				<up-swipe-action class="account_table">
					<up-swipe-action-item @click="(props) => conutClick(props, item, index)" :options="options"
						v-for="(item, index) in account.Items" :key="index">
              ...
              /> -->
							<up-avatar :src="item.icon_url ? item.icon_url : pic" size="40"
								style="border: 3px solid #60fbed"></up-avatar>
	...
	<CustomTabbar :selected="2" @change="tabberChange" @tabPage="tabPage"></CustomTabbar>
</template>
