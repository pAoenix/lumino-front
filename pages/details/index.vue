		...
		<view class="account_book_warpper">
			<up-sticky class="sticky_text" offset-top="10">
				<view style="height: 100%;
					...
					gap: 8px" @click="addAccountBook()">
					<up-icon name="plus-circle-fill" color="#55c9c9" size="20px"></up-icon>
					新建账本
				</view>
			</up-sticky>
			<view class="filter">
				<view class="filter_warpper">
					<view class="u-page__tag-item" v-for="(item, index) in radios" :key="index">
						<up-tag :class="{'u-tag-active': item.checked}" :text="item.text" :plain="!item.checked"
							shape="circle" :name="index" @click="radioClick(index)"
							:icon="!item.checked?'/static/sort.png':'/static/sort-active.png'">
						</up-tag>
					</view>
				</view>
				<view class="counts"><img class="counts_img" src="/static/sort.png" alt="" />跨账本统计</view>
			</view>
			<view class="account_book_table">
				<view class="table_for" v-for="(item, index) in accountList" :key="index">
					<view class="table_title">{{ item.createAt }} 创建</view>
					<view class="table_warpper">
						<view class="table_header" @click="toAccountPage(item)">
							<text>{{ item.title }}</text>
							<up-icon name="arrow-right"></up-icon>
						</view>
						<up-avatar-group class="avatar_warpper" :maxCount="7" :urls="getUserImg(item)" size="40"
							gap="0"></up-avatar-group>
						...
						<view class="table_foot">
							<view class="amount">收入 ¥<text class="amount_text">{{ item.incomeAmount }}</text> 元</view>
							<view class="amount">支出 ¥<text class="amount_text">{{ item.expensesAmount }}</text> 元</view>
						</view>
					</view>
				</view>
			</view>
